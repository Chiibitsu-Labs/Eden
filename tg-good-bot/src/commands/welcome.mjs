...
export const welcomeCommand = async (msg, bot, db) => {
    ...
    switch (action) {
        case 'on':
            db.data.communities[chatId].welcomeEnabled = true;
            await db.write();
            bot.sendMessage(msg.chat.id, "Welcome messages enabled.");
            break;
        case 'off':
            db.data.communities[chatId].welcomeEnabled = false;
            await db.write();
...
export const setWelcomeCommand = async (msg, bot, db) => {
    ...
    if (!db.data.communities[chatId]) {
        ...
        db.data.communities[chatId].welcomeMessage = newWelcomeMessage;
    }
